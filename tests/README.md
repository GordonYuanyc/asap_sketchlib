...
| `e2e/frequency.rs` / `e2e/cardinality.rs` / `e2e/quantiles.rs` | Deep per-family suites, each against its own theorem |
...
| `pbt/` | Property tests under `proptest`, one target (`cargo test --test pbt`): merge algebra, path equivalence, wire and ASAPv1 round-trips, bounded-heap invariants, Space-Saving bounds, OctoSketch delta promotion, HyperLogLog, and the papers' own reference models |
